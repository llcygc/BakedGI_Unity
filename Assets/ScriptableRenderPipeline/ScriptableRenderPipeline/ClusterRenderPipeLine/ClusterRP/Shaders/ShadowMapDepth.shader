...
Shader "Valve/ShadowCaster"
{
	SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature _ _ALPHATEST_ON _ALPHABLEND_ON _ALPHAPREMULTIPLY_ON _ALPHA_DITHERD_ON

			#pragma vertex MainVs
			#pragma fragment MainPs

			#include "UnityCG.cginc"
			//#include "CustomUnityStandardShadow.cginc"
			...
			VertexOutput MainVs(VertexInput i)
			{
				...
				o.vPositionPs.xyzw = UnityObjectToClipPos(float4(i.vPositionOs.xyz, 1.0));
	...
	SubShader
	{
		Tags{ "RenderType" = "treeTransparentCutout" "PerformanceChecks" = "False" }
		LOD 400
		...
		Pass
		{
			...
			float4 MainPs(VertexOutput i) : SV_Target
			{
				float4 color = tex2D(_MainTex, i.tex.xy);
