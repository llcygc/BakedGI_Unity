...
using Viva.Rendering.RenderGraph.ClusterPipeline.Attributes;
        ...
        public void RegisterLightingDebug()
        {
            ...
            if (lightingDebugSettings.shadowDebugMode == ShadowMapDebugMode.VisualizeShadowMap)
            {
                ...
                if (!lightingDebugSettings.shadowDebugUseSelection)
                    container.children.Add(new DebugUI.UIntField { displayName = "Shadow Map Index", getter = () => lightingDebugSettings.shadowMapIndex, setter = value => lightingDebugSettings.shadowMapIndex = value, min = () => 0u, max = () => (uint)(RenderPipelineManager.currentPipeline as ClusterRenderPipeline).GetCurrentShadowCount() - 1u });
            ...
            else if (lightingDebugSettings.shadowDebugMode == ShadowMapDebugMode.VisualizeAtlas)
            {
                list.Add(new DebugUI.Container
                {
                    children =
                    {
                        new DebugUI.UIntField { displayName = "Shadow Atlas Index", getter = () => lightingDebugSettings.shadowAtlasIndex, setter = value => lightingDebugSettings.shadowAtlasIndex = value, min = () => 0u, max = () => (uint)(RenderPipelineManager.currentPipeline as ClusterRenderPipeline).GetShadowAtlasCount() - 1u }
                    }
                });
            }
                            ...
                            new DebugUI.UIntField
                            {
                                ...
                                getter = () =>
                                {
                                    ...
                                    switch (fullScreenDebugMode)
                                    {
                                        case FullScreenDebugMode.FinalColorPyramid:
                                        case FullScreenDebugMode.PreRefractionColorPyramid:
                                            id = ClusterShaderIDs._GaussianPyramidColorMipSize;
                                            break;
                                        default:
                                            id = ClusterShaderIDs._DepthPyramidMipSize;
                                            break;
                                    }
            ...
            list.Add(new DebugUI.EnumField { displayName = "Tile/Cluster Debug", getter = () => (int)lightingDebugSettings.tileClusterDebug, setter = value => lightingDebugSettings.tileClusterDebug = (ClusterPass.LightManager.TileClusterDebug)value, autoEnum = typeof(ClusterPass.LightManager.TileClusterDebug), onValueChanged = RefreshLightingDebug });
            if (lightingDebugSettings.tileClusterDebug != ClusterPass.LightManager.TileClusterDebug.None && lightingDebugSettings.tileClusterDebug != ClusterPass.LightManager.TileClusterDebug.MaterialFeatureVariants)
            {
                list.Add(new DebugUI.Container
                {
                    children =
                    {
                        new DebugUI.EnumField { displayName = "Tile/Cluster Debug By Category", getter = () => (int)lightingDebugSettings.tileClusterDebugByCategory, setter = value => lightingDebugSettings.tileClusterDebugByCategory = (ClusterPass.LightManager.TileClusterCategoryDebug)value, autoEnum = typeof(ClusterPass.LightManager.TileClusterCategoryDebug) }
                    }
                });
            }
