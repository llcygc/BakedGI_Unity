...
#if defined(EVALUATE_SH_VERTEX) || defined(EVALUATE_SH_MIXED)
    half4 interpolators7 : TEXCOORD7;
#endif
...
#ifdef VARYINGS_NEED_CLUSTERID
    ouput.interpolators1 = input.clusterPos;
#endif

#ifdef VARYINGS_NEED_TANGENT_TO_WORLD
    output.interpolators2 = input.normalWS;
    output.interpolators3 = input.tangentWS;
#endif
...
#if defined(EVALUATE_SH_VERTEX) || defined(EVALUATE_SH_MIXED)
    output.interpolators7 = input.vertexSH;
#endif
...
#ifdef VARYINGS_NEED_CLUSTERID
    ouput.clusterPos = input.interpolators1.xyz;
#endif

#ifdef VARYINGS_NEED_TANGENT_TO_WORLD
    float4 tangentWS = float4(input.interpolators3.xyz, input.interpolators3.w > 0.0 ? 1.0 : -1.0);	// must not be normalized (mikkts requirement)

    // Normalize normalWS vector but keep the renormFactor to apply it to bitangent and tangent
	float3 unnormalizedNormalWS = input.interpolators2.xyz;
