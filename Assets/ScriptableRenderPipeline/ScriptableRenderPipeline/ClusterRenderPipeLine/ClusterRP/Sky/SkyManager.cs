...
namespace Viva.Rendering.RenderGraph
{
    ...
    public class BuiltinSkyParameters
    {
        ...
        public BuiltinSkyParameters()
        {
            pixelCoordToViewDirMatrix = new Matrix4x4[2];
            invViewProjMatrix = new Matrix4x4[2];
            cameraPosWS = new Vector3[2];
        }
    }
    ...
    public class SkyManager
    {
        ...
        void RebuildSkyMatrices(float nearPlane, float farPlane)
        {
            ...
            for (int i = 0; i < 6; ++i)
            {
                ...
                m_faceCameraInvViewProjectionMatrix[i] = RenderGraphUtils.GetViewProjectionMatrix(lookAt, cubeProj).inverse;
            }
        }
        ...
        {
            Shader.SetGlobalTexture(RenderGraphShaderIDs._SkyTexture, m_SkyboxGGXCubemapRT);
            float mipCount = Mathf.Clamp(Mathf.Log((float)m_SkyboxGGXCubemapRT.width, 2.0f) + 1, 0.0f, 6.0f);
            Shader.SetGlobalFloat(RenderGraphShaderIDs._SkyTextureMipCount, mipCount);
        }
        ...
        public void Build(ClusterRenderPipelineResources renderPipelinesResources)
        {
        ...
        void RenderSkyToCubemap(BuiltinSkyParameters builtinParams, SkySettings skySettings, RenderTexture target)
        {
            for (int i = 0; i < 6; ++i)
            {
                builtinParams.pixelCoordToViewDirMatrix[0] = m_facePixelCoordToViewDirMatrices[i];
                builtinParams.invViewProjMatrix[0] = m_faceCameraInvViewProjectionMatrix[i];
                ...
                m_Renderer.RenderSky(builtinParams, false, true);
            }
