Shader "Hidden/PostProcessing/Uber"
{
    HLSLINCLUDE
		...
		TEXTURE2DARRAY_SAMPLER2DARRAY(_Bloom_DirtTex, sampler_Bloom_DirtTex);
#else
        ...
        half4 FragUber(VaryingsDefault i) : SV_Target
        {
...
#ifdef UNITY_STEREO_INSTANCING_ENABLED
                    half4 s = SAMPLE_TEXTURE2D_ARRAY(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(Distort(pos)), sliceIndex);
#else
...
#ifdef UNITY_STEREO_INSTANCING_ENABLED
                half4 texelA = SAMPLE_TEXTURE2D_ARRAY(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(Distort(uv)), sliceIndex);
                half4 texelB = SAMPLE_TEXTURE2D_ARRAY(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(Distort(delta + uv)), sliceIndex);
                half4 texelC = SAMPLE_TEXTURE2D_ARRAY(_MainTex, sampler_MainTex, UnityStereoTransformScreenSpaceTex(Distort(delta * 2.0 + uv)), sliceIndex);
#else
            ...
            {
#ifdef UNITY_STEREO_INSTANCING_ENABLED
                color = SAMPLE_TEXTURE2D_ARRAY(_MainTex, sampler_MainTex, uvStereoDistorted, sliceIndex);
#else
            ...
            #if BLOOM
            {
                half4 bloom = UpsampleTent(_BloomTex, sampler_BloomTex, uvDistorted, _BloomTex_TexelSize.xy, _Bloom_Settings.x, sliceIndex);
                ...
                // is active
#ifdef UNITY_STEREO_INSTANCING_ENABLED
                half4 dirt = half4(SAMPLE_TEXTURE2D_ARRAY(_Bloom_DirtTex, sampler_Bloom_DirtTex, uvDistorted * _Bloom_DirtTileOffset.xy + _Bloom_DirtTileOffset.zw, sliceIndex).rgb, 0.0);
#else
